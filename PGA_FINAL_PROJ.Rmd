...
data<- iris
data2 <- newdata %>% filter(tournament.name == "Valspar Championship")%>%group_by(pos == 1)%>%summarise(winner = )
data <- masters %>% mutate(player = as.factor(player))
ggparcoord(data,
    columns = 15:20, groupColumn = 9
    ) 
# Plot
ggparcoord(data,
