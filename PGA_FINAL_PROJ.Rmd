...
ScottyP_WM <- Scotty_P %>% filter(tournament.name == "Waste Management Phoenix Open")%>%group_by(player)%>%group_by(made_cut)%>%summarise(ncut_made = n())

sScotty_WM <- Scotty %>%group_by(made_cut)%>%summarise(ncut_made_avg = n())
