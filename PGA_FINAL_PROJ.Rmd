...
```{r}
Scotty_P <- newdata %>% filter(player == "Scottie Scheffler" |player == "Patrick Cantlay")%>% mutate(made_cut = if_else(made_cut == 1,
                          true = "Made cut",
                          false = "Missed cut"))
...
ScottyP_WM <- Scotty_P %>% filter(tournament.name == "Waste Management Phoenix Open")%>%group_by(player)%>%summarise(ncut_made = n())
Scotty_WM <- Scotty %>%group_by(made_cut)%>%summarise(ncut_made_avg = n())
...
ggplot(data = Scotty_WM, aes(x= made_cut)) + 
 ...
 ggplot(ScottyP_WM, aes(x = made_cut, y = ncut_made, fill = m)) +
  geom_col()+
  scale_fill_manual(values = c("Made cut" = "green", "Missed cut" = "red"))
...
server <- function(input, output, session) {
  ...
  output$minorplot <- renderPlot({
    ...
    ggplot(data = df_update(), aes(x = play_yr, y = strokes))+
      geom_point(size=3, color="blue", fill=alpha("yellow", 0.3), alpha=0.7, shape=21, stroke=2) + 
      geom_segment( aes(x= play_yr, xend= play_yr, y=0, yend=strokes))+
      coord_flip()+
      geom_text(aes(label = strokes), vjust = 1.5, colour = "black")+
      ylim(0,288)
...
```{r}
Player <- newdata %>% filter(player == "Brian Harman") %>% summarise(putt_avg = mean(sg_putt, na.rm = TRUE))

Player2 <- newdata %>%summarise(green_avg = mean(sg_arg, na.rm = TRUE))

Player <- left_join(Player,Player2)

summarise(approach_avg = mean(sg_app, na.rm = TRUE))%>%
summarise(Tee_avg = mean(sg_ott, na.rm = TRUE))%>%
summarise(nonPutt_avg = mean(sg_t2g, na.rm = TRUE))%>%
summarise(Total_avg = mean(sg_total, na.rm = TRUE))
